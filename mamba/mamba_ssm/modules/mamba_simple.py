...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        xz = rearrange(
            ...
            "d (b l) -> b d l",
            l=seqlen,
        )  # torch.Size([8, 3072, 1024])
        if self.in_proj.bias is not None:
            xz = xz + rearrange(self.in_proj.bias.to(dtype=xz.dtype), "d -> d 1")

        A = -torch.exp(self.A_log.float())  # (d_inner, d_state)  torch.Size([1536, 16])
        # In the backward pass we write dx and dz next to each other to avoid torch.cat
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v3":
                A_b = -torch.exp(self.A_b_log.float())  # torch.Size([1536, 16])
                ...
                A_s = -torch.exp(self.A_s_log.float())  # torch.Size([1536, 16])

                xz_s = xz.chunk(self.nframes, dim=-1)
                xz_s = torch.stack(xz_s,dim=-1)  # torch.Size([8, 3072, 128, 8])
                xz_s = xz_s.flatten(-2)  # torch.Size([8, 3072, 1024])
            ...
            out = self.out_proj(y)
        return out  # torch.Size([8, 1024, 768])
