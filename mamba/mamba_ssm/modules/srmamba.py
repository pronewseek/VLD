...
class TransposeTokenReEmbedding:
    ...
    def transpose_normal_padding(x, rate):
        x = rearrange(x, "b c l -> b l c")  # torch.Size([8, 2048, 1024]) torch.Size([8, 1024, 2048])
        B, N, C = x.shape
        value = N // rate  # 1024 // 5 = 204
        if N % rate != 0:
            padding_length = (value + 1) * rate - N  # 1
            padded_x = torch.nn.functional.pad(x, (0, 0, 0, padding_length))  # torch.Size([8, 1025, 2048])
        else:
            padded_x = x
        x_ = rearrange(padded_x, "b (k w) d -> b (w k) d", w = rate)  # torch.Size([8, 1025, 2048])
        x_ = rearrange(x_, "b l c -> b c l")  # torch.Size([8, 2048, 1025])
        return x_
...
class SRMamba(nn.Module):
    def __init__(
        ...
        self.d_inner = int(self.expand * self.d_model)  # 1024
        self.dt_rank = math.ceil(self.d_model / 16) if dt_rank == "auto" else dt_rank  # 32
    ...
    def forward(self, hidden_states, inference_params=None, rate=10):  # rate=5
        """
        hidden_states: (B, L, D)
        ...
        A = -torch.exp(self.A_log.float())  # (d_inner, d_state)  torch.Size([1024, 16])
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            ...
            # 直接末尾补零并转置: TMamba
            N, C, L = xz.shape  # torch.Size([8, 2048, 1024])
            ...
            out_b = mamba_inner_fn_no_out_proj(
                    ...
                    delta_softplus=True,
                )  # torch.Size([8, 1024, 1025])
            # 将序列转置，并删除padding的位置
            out_b = TransposeTokenReEmbedding.transpose_remove_padding(out_b, rate=rate, length=L)  # torch.Size([8, 1024, 1024])
