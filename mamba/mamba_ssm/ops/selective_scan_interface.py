...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        #     dconv1d_out,
        #     dx, None, None, None,  # delta 参数占位  # 用这种修改，把这一行中的 dx 改为 None 就不会报错，但是 测试指标数值 有问题
        #     True, True
        # )

        ################## 修改 #####################
        # # 转为 channel last 格式 (B, L, C)
        # x_cl = x.transpose(1, 2).contiguous()
        # dconv1d_out_cl = dconv1d_out.transpose(1, 2).contiguous()
        #
        # # 调用 CUDA 后端 backward 函数
        # dx_cl, dconv1d_weight, dconv1d_bias, _ = causal_conv1d_cuda.causal_conv1d_bwd(
        #     x_cl, conv1d_weight, conv1d_bias,
        #     dconv1d_out_cl,
        #     None, None, None, None,  # delta 参数占位
        #     True, True
        # )
        #
        # # 转回 channel first (B, C, L)
        # dx = dx_cl.transpose(1, 2).contiguous()
        ...
        # dconv1d_out_cl = dconv1d_out.transpose(1, 2).contiguous()
        #
        # # 调整 conv1d_weight 形状为 (C, W)
        # if conv1d_weight.dim() == 3:
        #     # 常见形状是 [C, 1, W] 或 [C, C, W]
        #     if conv1d_weight.size(1) == 1:
        #         conv1d_weight = conv1d_weight.squeeze(1).contiguous()
        #     elif conv1d_weight.size(0) == conv1d_weight.size(1):
        #         conv1d_weight = conv1d_weight[:, 0, :].contiguous()
        #
        # # 调用 CUDA 后端 backward 函数
        # dx_cl, dconv1d_weight, dconv1d_bias, _ = causal_conv1d_cuda.causal_conv1d_bwd(
        #     x_cl, conv1d_weight, conv1d_bias,
        ...
        ################## 修改 #####################
        # out = causal_conv1d_cuda.causal_conv1d_bwd(
        #     x, conv1d_weight, conv1d_bias,
        #     dconv1d_out,
        #     None, None, None, None,
        #     True, True
        # )
        # dx, dconv1d_weight, dconv1d_bias = out[:3]
